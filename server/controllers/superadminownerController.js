...
exports.getOwnerEarnings = async (req, res) => {
  try {
    ...
    console.log(`[PHASE 3] Owner Earnings - Source: ${cacheResult.source}, Time: ${cacheResult.time}ms`);

    res.status(200).json({
      success: true,
      meta: {
        optimized: true,
        phase2QueryTime: '~1200ms (first load)',
        phase3CacheTime: `${cacheResult.time}ms (${cacheResult.source})`,
        queriesReduced: '400-600 → 1 aggregation + cache',
        cacheStats: cacheResult.stats
      },
      owners: cacheResult.data
    });
  } catch (error) {
