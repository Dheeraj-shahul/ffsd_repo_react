...
const Payments = () => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      if (appliedFilters.userName) {
        const term = appliedFilters.userName.toLowerCase();
        filtered = filtered.filter(p => p.userName?.toLowerCase().includes(term));
      }
      if (appliedFilters.minAmount) {
        filtered = filtered.filter(p => p.amount >= Number(appliedFilters.minAmount));
      }
      if (appliedFilters.maxAmount) {
        filtered = filtered.filter(p => p.amount <= Number(appliedFilters.maxAmount));
      }
      if (appliedFilters.status) {
        filtered = filtered.filter(p => p.status === appliedFilters.status);
      }
      if (appliedFilters.method) {
        filtered = filtered.filter(p => p.paymentMethod === appliedFilters.method);
      }
  ...
  const resetFilters = () => {
    const reset = {
      userName: '', minAmount: '', maxAmount: '', status: '', method: '',
  ...
  const handleLimitChange = (limit) => {
    ...
    setLocalFilters(prev => ({ ...prev, limit: newLimit, page: 1 }));
    const params = new URLSearchParams(searchParams);
      ...
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        ...
        <aside style={{
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {/* 1. User Name */}
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: '500' }}>User Name</label>
              <input
                type="text"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                ...
                <input
                  ...
                  style={{ width: '110px', padding: '10px', borderRadius: '8px', border: '1px solid #d5d9d9' }}
                />
              </div>
              <div>
          ...
          <div style={{
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', minWidth: '1400px', tableLayout: 'fixed' }}>
                ...
                <tbody>
                  {payments.length === 0 ? (
                    <tr><td colSpan="8" style={{ textAlign: 'center', padding: '60px', color: '#555' }}>No payments found.</td></tr>
                  ) : (
                    payments.map(p => (
                      <tr key={p._id || p.id} style={{ borderBottom: '1px solid #eaeded' }}>
                        <td style={{ padding: '14px 12px' }}>{(p._id || p.id)?.slice(-6)}</td>
                        <td><a href={`/admin/user/${p.tenantId || p.userId}/tenant`} style={{ color: '#0066cc' }}>{p.userName || 'Unknown'}</a></td>
                        <td style={{ fontWeight: '600', color: '#b12704' }}>₹{p.amount?.toLocaleString()}</td>
                        <td>
                          <span style={{
                        ...
                        <td>
                          <div className={styles['action-buttons']}>
                            <a href={`/admin/payment/${p._id || p.id}`} style={{ background: '#007bff', padding: '6px 10px', color: 'white', borderRadius: '6px', textDecoration: 'none', fontSize: '12px' }}>View</a>
                          </div>
                        </td>
                      </tr>
                    ))
            ...
            <div style={{
              ...
              <select value={appliedFilters.limit} onChange={e => handleLimitChange(e.target.value)}
