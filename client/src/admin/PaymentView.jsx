...
const StatusBadge = ({ status }) => {
  const map = { Paid:'badge-green', Pending:'badge-yellow', Overdue:'badge-red', Failed:'badge-red', Refunded:'badge-blue' };
...
.uv-grid-2{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.25rem}
...
const PaymentView = () => {
  ...
  const tenantName = payment.userName || (tenant ? `${tenant.firstName||''} ${tenant.lastName||''}`.trim() : '—');
  const propName   = payment.propertyName || property?.name || '—';
    ...
    <>
      ...
      <div className="uv-root">
        ...
        <div className="uv-topbar">
          <Link to="/admin/payments" className="uv-back">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
            Payments
          </Link>
          <div className="uv-topbar-actions">
            {payment.status === 'Paid'   && <button className="uv-btn uv-btn-danger"  onClick={handleRefund}>Refund</button>}
            {payment.status === 'Failed' && <button className="uv-btn uv-btn-success" onClick={handleRetry}>Retry</button>}
          </div>
        </div>
        ...
        <div className="uv-hero">
          ...
          <div className="uv-hero-info">
            <div className="uv-hero-name-row">
              <h1 className="uv-hero-name">{money(payment.amount)}</h1>
              <span className="uv-type-chip">Payment</span>
            </div>
            <p className="uv-hero-email">
              {property?._id ? <ULink to={`/admin/property/${property._id}`}>{propName}</ULink> : propName}
              {' · '}
              {tenant?._id ? <ULink to={`/admin/user/${tenant._id}/tenant`}>{tenantName}</ULink> : tenantName}
            </p>
            <div className="uv-hero-badges">
              <StatusBadge status={payment.status}/>
              {payment.paymentMethod && <span className="badge badge-gray">{payment.paymentMethod}</span>}
            </div>
          </div>
        ...
        <div className="uv-layout">
          ...
          <aside className="uv-sidebar">
            ...
            {(payment.status === 'Paid' || payment.status === 'Failed') && (
              <Card title="Actions">
                <div style={{display:'flex',flexDirection:'column',gap:'.75rem'}}>
                  {payment.status === 'Paid'   && <button className="uv-btn uv-btn-danger"  onClick={handleRefund} style={{justifyContent:'center'}}>↩ Issue Refund</button>}
                  {payment.status === 'Failed' && <button className="uv-btn uv-btn-success" onClick={handleRetry} style={{justifyContent:'center'}}>↻ Retry Payment</button>}
                </div>
              </Card>
            )}
            ...
            <Card title="Quick Links">
              <div className="uv-quick-links">
                <Link to="/admin/payments"     className="uv-quick-link">💳 All Payments</Link>
          ...
          <main className="uv-main">
            ...
            <Card title="Property Information">
              {property?._id ? (
                <div className="uv-grid-3">
                  <Field label="Property" value={<ULink to={`/admin/property/${property._id}`}>{property.name||propName}</ULink>}/>
                  <Field label="Location" value={property.location||'—'}/>
                  <Field label="Address"  value={property.address||'—'}/>
                </div>
              ) : propName !== '—' ? (
                <Field label="Property Name" value={propName}/>
            ...
            {booking?._id && (
              <Card title="Booking Reference">
                <div className="uv-grid-3">
                  <Field label="Booking ID" value={<ULink to={`/admin/booking/${booking._id}`}>{booking._id}</ULink>}/>
                  {booking.startDate && <Field label="Start Date" value={fmt(booking.startDate)}/>}
                  {booking.endDate   && <Field label="End Date"   value={fmt(booking.endDate)}/>}
                </div>
              </Card>
            )}
              ...
              <div className="uv-grid-3">
                ...
                {payment.receiptUrl && (
                  <Field label="Receipt" value={
                    <a href={payment.receiptUrl} target="_blank" rel="noopener noreferrer" className="uv-link">View Receipt ↗</a>
                  }/>
                )}
              </div>
            </Card>
